...
![Three side-by-side panels on black: a failover mechanism with one path rerouting around a broken model, a load-balancer fanning equal traffic into three identical squares, and a prompt-aware router inspecting the request through a magnifying glass before choosing between three differently-shaped models.](/blog/images/autorouting/01-three-kinds-of-routing.jpg)
...
![Side-by-side flow diagrams. Left panel "ROUTING": request goes through "predict difficulty" into a single chosen model and out to an answer. Right panel "CASCADING": request hits a cheap model first, a confidence check branches "yes" to an answer or "no" to a strong model. Caption: "Routing predicts once. Cascading escalates only on failure."](/blog/images/autorouting/02-routing-vs-cascading.jpg)
...
<Terminal title="tj optimize — downsize">
<span class="tline"><span class="t-prompt">$ </span><span class="t-cmd">tj optimize</span></span>
<span class="tline t-blank"> </span>
<span class="tline t-dim">Analyzing 247 sessions, 9.8K spans, 4.0M tokens (last 30d)…</span>
<span class="tline t-blank"> </span>
<span class="tline t-label">  ① Model downgrade: 47% of sessions look Haiku-eligible</span>
<span class="tline t-val">     • 116 of 247 sessions match structural heuristics</span>
<span class="tline t-val">     • Currently: claude-opus-4-7, claude-sonnet-4-7</span>
<span class="tline t-val">     • Suggested target: claude-haiku-4-5</span>
<span class="tline t-success">     • At current usage: $1,886 → $254 (-86% on flagged sessions)</span>
<span class="tline t-success">     • Estimated monthly savings: $1,632 (-38% total)</span>
<span class="tline t-blank"> </span>
<span class="tline t-warn">     ⚠ Structural heuristic only. Run `tj optimize --validate`</span>
<span class="tline t-warn">       to replay-test on your actual sessions before applying.</span>
</Terminal>
...
<span class="tline"><span class="t-prompt">$ </span><span class="t-cmd">tj onboard --claude-code</span></span>
<span class="tline"><span class="t-prompt">$ </span><span class="t-cmd">tj optimize downsize</span></span>
</Terminal>
...
- [TokenJam Downsize](/products/downsize). The measurement step before any router. Reads your own session history and flags the calls a cheaper model would have handled.
