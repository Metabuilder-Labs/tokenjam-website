...
![Peter Steinberger's June 7, 2026 post: "you shouldn't be prompting coding agents anymore. You should be designing loops that prompt your agents."](/blog/images/agent-loops/01-steinberger-tweet.png)
*The six-word post that put the timeline in a chokehold. (screenshot)*
...
![Two panels: a one-shot "human → prompt → agent → output" flow on the left, and a repeating "goal → plan → act → verify → repair → stop/ask" loop on the right.](/blog/images/agent-loops/02-prompting-vs-loop.png)
*One-shot prompting versus the loop the agent runs inside. (screenshot)*
...
![Timeline of agent loops: ReAct (2022), Self-Prompting / AutoGPT (2023), the Ralph loop (2025), productized /goal (2026), and multi-agent orchestration (now).](/blog/images/agent-loops/03-loop-lineage-timeline.png)
*Five rungs, oldest to newest. (screenshot)*
...
![Side-by-side comparison: open loops are exploratory and burn tokens on an unlimited budget; closed loops are bounded by a human-designed path with evals and stop conditions and run on a normal budget.](/blog/images/agent-loops/04-open-vs-closed.png)
*Open versus closed: the distinction that decides whether you can afford to run it. (screenshot)*
...
![A conceptual illustration of a loop with a quality gate: work passes through a checkmark gate to ship, or is bounced back to repair if it fails.](/blog/images/agent-loops/05-verification-gate.png)
...
![A developer's tweet: "Every ai agent i shipped this year is a for-loop, an llm call, and a try/catch around the json parsing. The only thing agentic about it is the anthropic bill at the end of the month."](/blog/images/agent-loops/06-cost-tweet.png)
*The cost punchline, from someone with their hands on the keyboard. (screenshot)*
